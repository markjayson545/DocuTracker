document.addEventListener('DOMContentLoaded', function() {
    ...
    function uploadDocument() {
        ...
        fetch('../backend/client/identity-verification/upload-verification-document.php', {
            method: 'POST',
            body: formData
        })
    ...
    function fetchUploadedDocuments() {
        fetch('../backend/client/identity-verification/get-uploaded-documents.php')
            .then(response => response.json())
            .then(data => {
                if (data.status === 'success') {
    ...
    function renderDocumentsTable(documents) {
        ...
        documents.forEach(doc => {
            ...
            let previewHtml = '';
            if (["jpg","jpeg","png"].includes(fileExt)) {
                previewHtml = `<img src="../backend/client/identity-verification/view-document.php?path=${encodeURIComponent(doc.document_path)}" style="max-width:60px;max-height:60px;">`;
            } else if (fileExt === 'pdf') {
                previewHtml = `<a href="../backend/client/identity-verification/view-document.php?path=${encodeURIComponent(doc.document_path)}" target="_blank"><i class='fas fa-file-pdf'></i> View</a>`;
            }
            tableBody.innerHTML += `
        ...
        document.querySelectorAll('.delete-document-btn').forEach(btn => {
            btn.onclick = function() {
                ...
                if (confirm('Delete this document?')) {
                    fetch('../backend/client/identity-verification/delete-document.php', {
                        method: 'POST',
                        headers: { },
        ...
        document.querySelectorAll('.update-document-btn').forEach(btn => {
            btn.onclick = function() {
                ...
                fileInput.onchange = function() {
                    ...
                    fetch('../backend/client/identity-verification/update-document.php', {
                        method: 'POST',
                        body: formData
                    })
                    .then(r => r.json())
