...
`;
    ...
    documentUploadForm.addEventListener("submit", function (event) {
        ...
        fetch('../backend/admin/admin-manage-requests/upload-request-document.php', { method: 'POST', body: formData })
            .then(res => res.json())
            .then(data => {
                console.log(data);
                if (data.success) {
    ...
    function handleRequestStatus(action, message = '') {
        ...
        fetch('../backend/admin/admin-manage-requests/handle-request-status.php', {
            method: 'POST',
            body: formData
        })
